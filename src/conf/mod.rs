...
impl Kind {
    ...
    fn config_paths(&self) -> Vec<String> {
        match self {
            Self::Api(_) => {
                vec!["/etc/basecoat/basecoat.toml".to_string()]
            }
            Self::Cli(_) => {
                #[allow(deprecated)]
                let user_home = home_dir().unwrap();
                let first_location = user_home.to_string_lossy() + "/.basecoat.toml";
                let second_location = user_home.to_string_lossy() + "/.config/basecoat.toml";
